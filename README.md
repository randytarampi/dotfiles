...
Without `DOTFILES_PROJECT_PRESET` the project inherits the global tier as-is and no
`opencode.json` is written; the file is generated only when the project overrides the
global tier or sets local-fallback inputs. For per-key overrides, hand-author a minimal
`opencode.json` — OpenCode merges it over the global config.
...
To undo project configuration, run `make project-cleanup PROJECT_ROOT=<project-dir>`
(`DRY_RUN=1` to preview; add `FORCE=1` to skip the confirmation prompt). It removes
artifacts generated by `configure-project.py` and never touches user-authored files
(`.opencode/.env`, `.ai/plans/`, hand-authored configs, runtime files under `.opencode/`).
...
│   ├── workflows/ci.yml
│   └── dependabot.yml
├── .chezmoiscripts/              # 32 scripts: four run_once (01-03, 21) + 28 run_onchange_04-29 (hash-triggered)
│   ├── # Phase 1: One-time setup (01-03, 21)
│   ├── # Phase 2: Package/CLI installs (04-11)
│   └── # Phase 3: Tool configuration (12-20, 22-29)
...
Use `scripts/run-local-review.sh --help` for the local pre-push review runner; details are in [docs/AGENTIC-REVIEW.md](docs/AGENTIC-REVIEW.md).

Fifteen presets for AI agents, defined in `configs/opencode/oh-my-opencode-slim.json` and resolved through `scripts/lib/tier_registry.py` (the shared source of truth). See [docs/TIERS.md](docs/TIERS.md) for the full tier reference:
...
| **free** | OpenCode Zen + Google + OpenRouter (all free tiers) | Free cross-provider workflow with cross-provider fallbacks |
