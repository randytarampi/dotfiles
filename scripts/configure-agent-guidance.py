...
def inject(agent_path, guidance, dry_run=False, force=False, check=False):
  ...
  if os.path.islink(agent_path):
    ...
    if not os.path.exists(target):
      if check:
        logger.error(f"BROKEN SYMLINK: {agent_path} -> {target} (would remove and replace with --force)")
        return False
      if dry_run:
        logger.info(f"[DRY RUN] Would remove broken symlink: {agent_path} -> {target}")
      else:
  ...
  if MARKER_START in content and MARKER_END in content:
    ...
    if start_idx >= len(header_prefix) and content[start_idx - len(header_prefix):start_idx] == header_prefix:
  ...
  if LEGACY_MARKER_START in content and LEGACY_MARKER_END in content:
    if check:
      logger.error(f"DRIFT: {agent_path} (has legacy CODEGRAPH markers, needs migration)")
      return False
  ...
  if force:
    if check:
      logger.error(f"MISSING: {agent_path} (no markers, would append with --force)")
      return False
...
def main():
  ...
  for path in AGENT_FILES:
    result = inject(path, guidance, dry_run=args.dry_run, force=args.force, check=args.check)
    if result:
      updated += 1
    elif args.check:
      drift = True
  ...
  if args.check:
    ...
    logger.info(f"[DRY RUN] Agent guidance would be injected into {updated}/{total} agent files")
  else:
