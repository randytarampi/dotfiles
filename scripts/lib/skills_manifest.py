...
def load_manifest(manifest_path: str) -> dict:
    """Load and parse the skills manifest JSON.

    Returns the raw manifest dict.
    """
...
def get_symlinked_skills(target_dir: str) -> set[str]:
    ...
    return {
        ...
        if os.path.isdir(os.path.join(target_dir, entry)) and not entry.startswith(".")
    }
...
def fetch_skill_via_cli(source: str, name: str, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        # CLI may return non-zero if some agents fail (e.g. PromptScript),
        # but still successfully install to ~/.agents/skills/. Check if the
        # skill directory exists as a success signal.
