...
| 29 | configure-omlx | run_onchange | oMLX nested settings generation, Metal wired-limit tuning, and service startup | `DOTFILES_RUN_OMLX_SETUP` |
| 30 | configure-openwebui | run_onchange | Open WebUI pinned venv, LaunchAgent, service env, and connection reconciliation | `DOTFILES_RUN_OPENWEBUI_SETUP` |
| 31 | configure-litellm | run_onchange | Separate LiteLLM loopback gateway for non-OpenWebUI clients | `DOTFILES_RUN_LITELLM_SETUP` |
| 32 | openwebui-backup-timer | run_onchange | Daily Open WebUI backup LaunchAgent timer | `DOTFILES_RUN_OPENWEBUI_BACKUP_SCHEDULE` (requires Open WebUI gate) |
| — | configure-project | manual/project | Unified project-scoped configuration (`--steps` selects work; default `opencode,codegraph,skills,jetbrains,junie`; optional `pi` via `DOTFILES_PROJECT_PI`; the `opencode` step writes root `opencode.json` only when the project overrides the global tier or sets local-fallback inputs) | project `.opencode/.env` |
| — | cleanup-project | manual/project | Removes artifacts generated by configure-project.py (root `opencode.json`, `.opencode/*` generated files, `.codegraph/`, skill symlinks, `.junie`/`.aiassistant` links, `.pi/*`, generated MCP merges); never touches user-authored files (`.opencode/.env`, `.ai/plans/`, hand-authored configs) | `make project-cleanup PROJECT_ROOT=<dir>` |
| — | update-system | manual (`_dot--update-system`) | Cross-platform system/package updater: brew (macOS), apt/dnf/pacman (Linux), winget (Windows), npm globals, uv, pipx, pi, opencode, junie, ollama models, and gated oMLX model presence verification; Brewfile sync via sync-brewfiles.py | `DOTFILES_RUN_UPDATE_SYSTEM_SETUP` |
...
| `DOTFILES_RUN_OPENWEBUI_SETUP` | 0 | Script 30 (Open WebUI venv, LaunchAgent, service env, connection reconciliation) |
| `DOTFILES_RUN_LITELLM_SETUP` | 0 | Script 31 (separate LiteLLM loopback gateway) |
| `DOTFILES_RUN_OPENWEBUI_BACKUP_SCHEDULE` | 0 | Script 32 (daily Open WebUI backup timer; requires Open WebUI gate) |
